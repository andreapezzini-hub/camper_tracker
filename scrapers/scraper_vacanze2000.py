...
def regex_extract_camper_data(raw_text, current_price, db_conn):
    ...
    # Protezione per evitare false assegnazioni
    if tipo_furgonato:
        tipo_semintegrale = False
        tipo_motorhome = False
        tipo_mansardato = False
    elif tipo_semintegrale:
        # Priorità al semintegrale se nel testo sono presenti sia 'semintegrale' che 'mansardato'
        tipo_mansardato = False
        tipo_motorhome = False
...
def run_scraper(db_conn, config, ollama_config=None, skip_ai=False):
    SITE_NAME = "Vacanze 2000"
    BASE_URL = "https://www.vacanze2000.it"
    DISTANCE_FROM_SEREGNO = 15 
    
    urls_to_scrape = [
        "https://www.vacanze2000.it/camper-usati/",
        "https://www.vacanze2000.it/camper-nuovi/"
    ]
    ...
    try:
        ...
        for target_url in urls_to_scrape:
            ...
            for link in links_veicoli:
                ...
                # Risoluzione URL relativo/assoluto
                url_completo = href if href.startswith('http') else urljoin(BASE_URL, href)
                
                # Escludiamo link verso domini esterni
                ...
                # Logica estesa: gli annunci si trovano tipicamente dentro queste categorie/percorsi,
                # e il path deve avere almeno 2 livelli (es: /camper-usati/marca-modello/)
                valid_bases = ['camper-usati', 'camper-nuovi', 'prodotto', 'veicolo', 'veicoli', 'scheda', 'mezzo', 'mezzi', 'annuncio']
                
                is_valid = False
                if len(path_parts) >= 2 and any(base in path_parts for base in valid_bases):
                    is_valid = True
                    
                # Filtra esplicitamente paginazioni, ancore, categorie listato e link accessori
                exclusions = ['#', '/page/', '/categoria', '/category', '/tag', 'wp-json', '/contatti', '/chi-siamo', '/servizi', '/noleggio']
                if any(x in href_lower for x in exclusions): 
                    is_valid = False
                    
                # Filtra parametri di ordinamento o carrello, ma permetti query generiche (se usate per l'ID)
                if '?' in href_lower and any(param in href_lower for param in ['filter', 'orderby', 'add-to-cart', 'paged']): 
                    is_valid = False
                    
                if url_completo in processed_urls or url_completo == target_url: 
                    is_valid = False
                ...
                if is_valid:
                    ...
                    try:
                        ...
                        main_content = det_soup.find('main') or det_soup.find(class_='site-main') or det_soup
                        ...
                        img_url = None
                        img_tag = main_content.find('img', class_=re.compile(r'attachment|wp-post-image|woocommerce', re.I))
                        if not img_tag:
                        ...
                        # Utilizzo scraper_utils centralizzato
...
if __name__ == "__main__":
    ...
    # Helper per simulare DB SQLite durante il test isolato
    def get_test_db():
    ...
    print(f"Avvio test isolato Scraper Vacanze 2000 (SQLite)...")
